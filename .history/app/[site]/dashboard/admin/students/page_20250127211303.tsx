...
export default function Students() {
	...
	const feeRecords = Object.values(fees);
	const studentList = Object.values(students);

	const updatedStudents = studentList.map(student => {
	const feeRecord = feeRecords.find(fee => fee.studentId === student.id);
	const startDate = feeRecord?.createdAt || student.createdAt;
	console.log(startDate)
	return {
		...student,
		unpaidMonths: getMonthsBetweenDates(startDate, currentDate),
	};
	});

	function getMonthsBetweenDates(startDate:any, endDate) {
		const months = [];
		const current = new Date(startDate);
	  
		// Start from the next month after the startDate
		current.setMonth(current.getMonth() + 1);
		current.setDate(1); // Normalize to the 1st day of the next month
