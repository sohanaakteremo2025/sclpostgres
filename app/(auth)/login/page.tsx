...
export default function Login() {
	...
	const ErrorDisplay = ({ message }: { message: string }) => {
		...
		return (
			<Alert
				variant="destructive"
				className="flex items-center bg-red-50 border-red-200 dark:bg-red-900/20 dark:border-red-800 rounded-lg mb-4"
			>
			...
			<Form {...form}>
				<form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
					...
					<Button
						disabled={isPending}
						type="submit"
						className="w-full h-12 text-white bg-primary hover:bg-purple-800 rounded-lg font-medium text-base transition-all shadow-lg shadow-primary/20 hover:shadow-primary/30"
					>
						{isPending ? (
